...
let mainFunction = (config) => {
    ...
    useDistanceMetricCheckbox.checked = terrender.getParameters().useDistanceMetric;
    useDistanceMetricCheckbox.addEventListener('change', () => { terrender.getParameters().setParam('useDistanceMetric', useDistanceMetricCheckbox.checked) });

    let useGeomMetricCheckbox = document.querySelector('#useGeomMetric');
    useGeomMetricCheckbox.checked = terrender.getParameters().useGeomMetric;
    useGeomMetricCheckbox.addEventListener('change', () => { terrender.getParameters().setParam('useGeomMetric', useGeomMetricCheckbox.checked) });

    let useCullingMetricCheckbox = document.querySelector('#useCullingMetric');
    useCullingMetricCheckbox.checked = terrender.getParameters().useCullingMetric;
    useCullingMetricCheckbox.addEventListener('change', () => { terrender.getParameters().setParam('useCullingMetric', useCullingMetricCheckbox.checked) });

    let kPatchBaseDiv = document.querySelector('#kPatchBase');
    for (let i = (terrender.getParameters().tileSideLength - 1) / 2; i > 16; i = i / 2) {
        let option = document.createElement('option');
        option.value = i + 1;
        option.innerHTML = i + 1;
        if (i + 1 == terrender.getParameters().kPatchBase) {
            option.selected = true;
        }
        kPatchBaseDiv.appendChild(option);
    }
    kPatchBaseDiv.addEventListener('change', () => {
        terrender.getParameters().setParam('kPatchBase', kPatchBaseDiv.value);
    });


    let statsButton = document.querySelector('#writeStats');
    statsButton.addEventListener('click', () => {
        terrender.getTimer().writeAverageTimingsToConsole();
    });

    let maxLodDiv = document.querySelector('#maxLod');
    maxLodDiv.value = terrender.getParameters().currentLod;
    maxLodDiv.addEventListener('change', () => {
        let newVal = Number.parseInt(maxLodDiv.value);
        if (newVal >= 0 && newVal <= terrender.getParameters().maxLod) {
            terrender.getParameters().setParam('currentLod', newVal);
        } else {
            maxLodDiv.value = terrender.getParameters().currentLod;
        }
    });
    ...
    errorThresholdDiv.value = terrender.getParameters().errorThreshold;
    errorThresholdDiv.addEventListener('change', () => {
        let newVal = Number.parseFloat(errorThresholdDiv.value);
        if (newVal > 0.0) {
            terrender.getParameters().setParam('errorThreshold', errorThresholdDiv.value)
        } else {
            errorThresholdDiv.value = terrender.getParameters().errorThreshold;
        }
    });
