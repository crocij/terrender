...
void main() {
    ...
    if ( color.a == 1.0) {
        float xCoord = rgbaToFloat(texture(pixelPosTextureX, texCoord));  
        float yCoord = rgbaToFloat(texture(pixelPosTextureY, texCoord));
        ...
        if (linesLength > 0) {
            ...
            for (int i = 0; i < 30000; i++) {
                float lineIndex = rgbaToFloat(texture(bucketsTexture, vec2((float(i) + 0.5) * offsetWidthBuckets, bucketIndex)));

                if (lineIndex < 0.0 || i >= maxLinesBucket) break;
                float indexWidth = (lineIndex + 0.5) * offsetWidthLines;
                float p1x = rgbaToFloat(texture(linesTexture, vec2(indexWidth, 0.5 * offsetHeightLines)));
                float p1y = rgbaToFloat(texture(linesTexture, vec2(indexWidth, 1.5 * offsetHeightLines)));
                float p2x = rgbaToFloat(texture(linesTexture, vec2(indexWidth, 2.5 * offsetHeightLines)));
